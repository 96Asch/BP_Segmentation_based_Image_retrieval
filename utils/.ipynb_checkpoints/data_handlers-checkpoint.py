...
def dump_features(images_dir, labels, hdf5_path, feature_extractor, image_formats=("jpg")):
    print('Dumping features from images at [', str(images_dir), '] to [', hdf5_path, ']')
    image_paths = []
    ...
    imageIDDB = db.create_dataset("image_ids", shape=(len(labels),), dtype=h5py.special_dtype(vlen=str))
    ...
    db.close()
    print('Finished dumping features to: ', str(hdf5_path))


def extract_feature(image, feature_extractor, image_format=("jpg")):
